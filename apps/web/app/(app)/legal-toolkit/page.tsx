...
function CustomDocModal({ profileId, name, email, onClose, onCreated }: {
  profileId: string; name: string; email: string;
  onClose: () => void; onCreated: (c: Contract) => void;
}) {
  const [title,       setTitle]       = useState("");
  const [kind,        setKind]        = useState<TemplateKind>("nda");
  ...
  async function submit() {
    if (!title.trim() || !body.trim()) {
      setError("Title and document body are required.");
      return;
    }
    setLoading(true);
    setError("");
    try {
      ...
      const text =
        `${title.trim()}\nGenerated by AiStaff Legal Toolkit — ${date}\n` +
        `Party A: ${name}\n` +
        (partyBName.trim() ? `Party B: ${partyBName.trim()}\n` : "") +
        `\n${body.trim()}`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60">
      <div className="w-full sm:w-[560px] bg-zinc-900 border border-zinc-700 rounded-sm sm:rounded-sm mx-0 sm:mx-4">
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
          <span className="font-mono text-sm text-zinc-100">Custom Document</span>
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-300 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        <div className="px-4 py-4 space-y-3 max-h-[70vh] overflow-y-auto">
          {/* Title */}
          <div>
            <label className="block font-mono text-[11px] text-zinc-400 mb-1">Document title <span className="text-red-500">*</span></label>
            <input
              className="w-full h-8 px-2.5 bg-zinc-950 border border-zinc-700 rounded-sm font-mono text-xs text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-amber-500"
          ...
          {/* Type */}
          <div>
            <label className="block font-mono text-[11px] text-zinc-400 mb-1">Document type</label>
            <select
              className="w-full h-8 px-2.5 bg-zinc-950 border border-zinc-700 rounded-sm font-mono text-xs text-zinc-100 focus:outline-none focus:border-amber-500"
              value={kind}
              onChange={e => setKind(e.target.value as TemplateKind)}
            >
              {Object.entries(KIND_LABEL).map(([k, v]) => (
                <option key={k} value={k}>{v} — {TEMPLATES.find(t => t.kind === k)?.name ?? k}</option>
              ))}
            </select>
          </div>
          {/* Party B */}
          <div>
            <label className="block font-mono text-[11px] text-zinc-400 mb-1">Counterparty name <span className="text-zinc-600">(optional)</span></label>
            <input
              className="w-full h-8 px-2.5 bg-zinc-950 border border-zinc-700 rounded-sm font-mono text-xs text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-amber-500"
          ...
          {/* Body */}
          <div>
            <label className="block font-mono text-[11px] text-zinc-400 mb-1">Document body <span className="text-red-500">*</span></label>
            <textarea
              className="w-full px-2.5 py-2 bg-zinc-950 border border-zinc-700 rounded-sm font-mono text-xs text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-amber-500 resize-none leading-relaxed"
              rows={10}
              placeholder={"Write or paste your document text here.\n\nYou can include clauses, terms, conditions, schedules, etc.\n\nThis will be hashed and PDF-exported."}
              value={body}
              onChange={e => setBody(e.target.value)}
            />
            <p className="font-mono text-[10px] text-zinc-600 mt-1">{body.length} chars · SHA-256 hashed on creation</p>
          </div>
          {/* Counterparty email */}
          <div className="border-t border-zinc-800 pt-3">
            <label className="block font-mono text-[11px] text-zinc-400 mb-1">
...
function WarrantyTab({ profileId }: { profileId: string }) {
  ...
  if (claims.length === 0) return (
    <div className="space-y-3">
      <div className="border border-zinc-800 rounded-sm p-4 bg-zinc-900/30">
        <p className="font-mono text-xs text-zinc-200 mb-2 flex items-center gap-1.5">
          <ShieldCheck className="w-3.5 h-3.5 text-amber-400" /> What is a Warranty Claim?
        </p>
        <p className="font-mono text-[11px] text-zinc-400 leading-relaxed">
          A warranty claim is automatically raised when the telemetry service detects that a deployed agent
          has drifted from its verified baseline — e.g. unexpected output changes, failed health checks,
          or artifact hash mismatch.
        </p>
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2">
          {[
            { label: "7-day window",    desc: "Client has 7 days post-deployment to raise a claim" },
            { label: "3 resolutions",   desc: "Fix (REMEDIATED) · Refund (REFUNDED) · Deny (REJECTED)" },
            { label: "Escrow-linked",   desc: "Unresolved P0 claims freeze payout until resolved" },
          ].map(({ label, desc }) => (
            <div key={label} className="border border-zinc-800 rounded-sm p-2">
              <p className="font-mono text-[11px] text-amber-400 mb-0.5">{label}</p>
              <p className="font-mono text-[10px] text-zinc-500">{desc}</p>
            </div>
          ))}
        </div>
      </div>
      <p className="font-mono text-xs text-zinc-600 text-center py-4">No warranty claims on record.</p>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60">
      <div className="w-full sm:w-[480px] bg-zinc-900 border border-zinc-700 rounded-sm sm:rounded-sm mx-0 sm:mx-4">
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
          <span className="font-mono text-sm text-zinc-100">New Document</span>
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-300 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        <div className="px-3 py-3 space-y-1.5 max-h-[60vh] overflow-y-auto">
          <p className="font-mono text-[10px] text-zinc-600 uppercase tracking-widest px-1 pb-1">Standard Templates</p>
          {TEMPLATES.map(tpl => (
            <button
              key={tpl.id}
          ...
          <div className="border-t border-zinc-800 pt-2 mt-1">
            <p className="font-mono text-[10px] text-zinc-600 uppercase tracking-widest px-1 pb-1.5">Custom</p>
            <button
              onClick={() => setCustom(true)}
              className="w-full text-left flex items-start gap-3 px-3 py-2.5 rounded-sm border border-dashed border-zinc-700 hover:border-amber-700 hover:bg-amber-950/20 transition-colors"
            >
              <span className="font-mono text-[10px] px-1 py-0.5 border border-amber-800 text-amber-400 rounded-sm flex-shrink-0 mt-0.5">
                CUSTOM
              </span>
              <div className="min-w-0">
                <p className="font-mono text-sm text-zinc-100">Custom Document</p>
                <p className="font-mono text-[11px] text-zinc-500 mt-0.5">
                  Write or paste your own document. Free-form text, any structure — hashed, PDF-exported, e-signature ready.
                </p>
              </div>
            </button>
          </div>

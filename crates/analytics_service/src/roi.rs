...
fn compute_reputation(pass_pct: f64, total: i64, drift: i64, trust: i16) -> f64 {
    ...
    let score = 0.40 * pass_pct + 0.30 * drift_free_pct + 0.20 * trust_norm + 0.10 * vol_norm;
...
pub async fn talent_roi(db: &PgPool, talent_id: Uuid) -> Result<RoiReport> {
    ...
    match row {
        ...
        None => {
            ...
            let profile =
                sqlx::query("SELECT trust_score, display_name FROM unified_profiles WHERE id = $1")
                    .bind(talent_id)
                    .fetch_optional(db)
                    .await?;

...
async fn main() -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        // ── Community Events ──────────────────────────────────────────────────
        .route("/hubs/:hub_id/events",     get(handlers::list_hub_events).post(handlers::create_hub_event))
        .route("/hubs/:hub_id/events/:eid/rsvp", post(handlers::rsvp_event))
        // ── Forum Threads & Posts ─────────────────────────────────────────────
        .route("/hubs/:hub_id/threads",          get(handlers::list_threads).post(handlers::create_thread))
        .route("/hubs/:hub_id/threads/:tid",     get(handlers::get_thread))
        .route("/hubs/:hub_id/threads/:tid/posts", get(handlers::list_posts).post(handlers::create_post))
        // ── Mentor Profiles ───────────────────────────────────────────────────
        .route("/mentors",                get(handlers::list_mentors).post(handlers::upsert_mentor_profile))
        ...
        .route("/mentorship/pairs/:pid/sessions", get(handlers::list_sessions).post(handlers::schedule_session))
        .route("/mentorship/pairs/:pid/sessions/:sid/complete", post(handlers::complete_session))
        // ── Cohorts ───────────────────────────────────────────────────────────
        .route("/cohorts",             get(handlers::list_cohorts).post(handlers::create_cohort))
        .route("/cohorts/:cid/join",   post(handlers::join_cohort))
        // ── Career Growth ─────────────────────────────────────────────────────
        .route("/career/:user_id",        get(handlers::get_career_profile))
        .route("/career/:user_id/milestones", get(handlers::list_milestones).post(handlers::award_milestone))
        .route("/career/:user_id/gaps",   get(handlers::list_skill_gaps))
        .route("/career/:user_id/paths",  get(handlers::list_learning_paths).post(handlers::assign_learning_path))
        .route("/career/:user_id/paths/:path_id/progress", put(handlers::update_path_progress))
        // ── Well-Being ────────────────────────────────────────────────────────
